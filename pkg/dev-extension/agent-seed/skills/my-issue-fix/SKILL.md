...
- **Comments document public interfaces, nothing else.** `my-code-comment-refinement` enforces it, and `my-commit-create` runs it first: every comment the change adds is listed and judged, and only contract documentation on public interfaces survives. Write the fix that way from the start - a comment you are going to delete is a sign the code needed a better name. Rationale a reviewer needs belongs in the PR's Technical notes, not in the source.
